...
class DotfileManager:
    ...
    def _install_packages(self, packages: Dict[str, set]) -> bool:
        ...
        try:
            ...
            # Install official packages in groups to minimize sudo prompts
            if packages['pacman']:
                package_list = list(packages['pacman'])
                self.logger.info(f"Installing {len(package_list)} official packages...")
                ...
                for i in range(0, len(package_list), batch_size):
                    ...
                    self.logger.info(f"Installing batch: {', '.join(batch)}")
                    if not self._run_command(['pacman', '-S', '--needed', '--noconfirm'] + batch, sudo=True):
                        self.logger.error(f"Failed to install batch: {', '.join(batch)}")
                        return False

            # Install AUR packages
            if packages['aur']:
                # First check if we have an AUR helper
                aur_helper = None
                for helper in ['yay', 'paru']:
                    if self._check_package_installed(helper):
                        aur_helper = helper
                        break
                ...
                # Install AUR packages in batches
                package_list = list(packages['aur'])
                self.logger.info(f"Installing {len(package_list)} AUR packages using {aur_helper}...")
                
                for i in range(0, len(package_list), batch_size):
                    batch = package_list[i:i + batch_size]
                    self.logger.info(f"Installing AUR batch: {', '.join(batch)}")
                    if not self._run_command([aur_helper, '-S', '--needed', '--noconfirm'] + batch):
                        self.logger.error(f"Failed to install AUR batch: {', '.join(batch)}")
                        return False
    ...
    def _install_aur_helper(self) -> bool:
        ...
        try:
            ...
            if not self._run_command(['makepkg', '-si', '--needed', '--noconfirm'], cwd=clone_dir):
                return False
            ...
            self.logger.error(f"Failed to install AUR helper: {str(e)}")
            return False
    ...
    def _load_dependency_map(self) -> Dict[str, str]:
        """Load the dependency map from configs/dependency_map.json."""
        try:
            dependency_map_path = os.path.join(os.path.dirname(__file__), '..', 'configs', 'dependency_map.json')
            with open(dependency_map_path, 'r') as f:
                return json.load(f)
        except Exception as e:
            self.logger.error(f"Failed to load dependency map: {str(e)}")
            return {}
    ...
    def _detect_required_packages(self, local_dir: str) -> Dict[str, set]:
        ...
        try:
            ...
            if not dependency_map:
                self.logger.error("Failed to load dependency map")
                return required_packages
        except Exception as e:
            self.logger.error(f"Error loading dependency map: {str(e)}")
            return required_packages

        # Scan directory structure and map to packages
        for item in os.listdir(local_dir):
            # Check if the item is in our dependency map
            if item in dependency_map:
                package = dependency_map[item]
                # For now, assume all packages are from pacman
                # TODO: Add AUR package detection logic
                required_packages['pacman'].add(package)
                self.logger.info(f"Detected {item} configuration, adding package {package}")

            # Special handling for .oh-my-zsh plugins
            if item == '.oh-my-zsh':
                plugins_dir = os.path.join(local_dir, item, 'plugins')
                if os.path.exists(plugins_dir):
                    for plugin in os.listdir(plugins_dir):
                        if plugin in dependency_map:
                            package = dependency_map[plugin]
                            required_packages['pacman'].add(package)
                            self.logger.info(f"Detected zsh plugin {plugin}, adding package {package}")

        # Add font packages if GUI components are present
        if any(x in os.listdir(local_dir) for x in ['gtk-3.0', 'gtk-4.0', 'i3', 'polybar', 'kitty']):
